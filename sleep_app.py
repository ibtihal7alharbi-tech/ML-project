...
# ═══════════════════════════════════════════════════════════
#  MODELS  — trained only on features we collect from user
# ═══════════════════════════════════════════════════════════
...
def train_all(df_raw):
    ...
    avnum=[c for c in UNUM if c in df.columns]
    avcat=[c for c in UCAT if c in df.columns]

    def ohe(data,cats): return pd.get_dummies(data,columns=[c for c in cats if c in data.columns],drop_first=False)
...
def dis_full_desc(disorder,none_prob,stress,wk,lat):
    ...
    return f"Your sleep pattern shows some signs of disruption. Consistent habits over 2–3 weeks typically improve sleep quality significantly."
...
def get_sab_info(f1, u):
    """Return (name, [bullets], action) — all bullets use user's actual values."""
    stress=u.get("stress_score",5); caf=u.get("caffeine_mg_before_bed",0)
    screen=u.get("screen_time_before_bed_mins",0); wk=u.get("wake_episodes_per_night",1)
    lat=u.get("sleep_latency_mins",15); hrs=u.get("sleep_duration_hrs",7)
    temp=u.get("room_temperature_celsius",20); work=u.get("work_hours_that_day",8)
    steps=u.get("steps_that_day",7000); nap=u.get("nap_duration_mins",0)
    deep=u.get("deep_sleep_percentage",18); rem=u.get("rem_percentage",20)

    if f1=="stress_score":
        return ("You were stressed today",
            [f"Stress at {stress}/10 keeps your brain in alert mode — the opposite of what you need for deep sleep",
             "Your body produces a hormone (cortisol) that directly blocks the sleep hormone (melatonin)",
             f"At {stress}/10, expect lighter, more fragmented sleep tonight — this is the biggest thing to fix"],
            "Tonight: write down everything on your mind on paper, close the notebook. Then 4 seconds in, hold 4, out 6. Repeat 5 times.")
    if f1=="sleep_duration_hrs":
        return (f"You only slept {hrs:.1f} hours",
            [f"That's {max(0,7-hrs):.1f} hours under the 7-hour minimum your body needs to fully recover",
             "Every missing hour = one fewer repair cycle for your muscles, immune system, and memory",
             "Sleep debt builds up — last night's deficit makes tonight even more important"],
            f"Tonight: be in bed by {u.get('bedtime_hint', '10:30 PM')}. Set a bedtime alarm — not just a morning alarm.")
    if f1=="caffeine_mg_before_bed":
        cups=max(1,int(caf//95))
        return (f"Caffeine ({caf}mg) is still in your system",
            [f"You had {caf}mg — roughly {cups} coffee(s). Half of that is still active in your body right now",
             "Caffeine blocks the signal that tells your brain it's time to sleep — for 6+ hours",
             "It doesn't just delay sleep — it reduces the depth of your deep sleep even after you're asleep"],
            f"Tomorrow: nothing caffeinated after 1 PM. The earlier you stop, the deeper you'll sleep.")
    if f1=="screen_time_before_bed_mins":
        return (f"{screen} minutes of screens before bed",
            [f"At {screen} minutes, you significantly delayed your body's natural melatonin production",
             "Screen light tells your brain it's still daytime — even when you feel tired",
             "The scrolling itself keeps your mind stimulated right when it should slow down"],
            "Put your phone in another room 45 min before bed. Use a standalone alarm clock.")
    if f1=="wake_episodes_per_night":
        return (f"You woke up {wk} times last night",
            [f"Waking {wk} times means your sleep cycles kept resetting from the beginning",
             "The deepest, most restorative sleep happens in the middle of the night — repeated wakings cut it short",
             "Even brief wakings you don't fully remember reduce how rested you feel in the morning"],
            "Tonight: check room temperature, noise, light leaks, or fluids before bed. Fix the most likely cause.")
    if f1=="sleep_latency_mins":
        if lat<=20:
            return (f"You fell asleep quickly ({lat:.0f} min) — this is healthy",
                [f"Falling asleep in {lat:.0f} minutes is within the healthy 10–20 minute range",
                 "This suggests your body was ready for sleep at the right time",
                 "Keep your pre-bed routine consistent to maintain this"],
                "Keep your bedtime consistent — this healthy sleep onset will compound over time.")
        elif lat<=30:
            return (f"Sleep onset was slightly slow ({lat:.0f} min)",
                [f"It took {lat:.0f} minutes to fall asleep — slightly above the ideal 10–20 min",
                 "This can happen after a mildly stressful day or with some screen time before bed",
                 "Not a major concern, but a consistent wind-down routine would help"],
                "Try 5 minutes of calm breathing before bed: 4 seconds in, hold 4, out 6.")
        else:
            return (f"It took {lat:.0f} minutes to fall asleep — that's too long",
                [f"Over 30 minutes to fall asleep means your mind was still active at bedtime",
                 "This is often caused by stress, screens, or not having a wind-down routine",
                 "The longer this happens regularly, the more your brain associates bed with being awake"],
                "If still awake after 20 min: get up, do something calm in dim light, return when sleepy.")
    if f1=="rem_percentage":
        return (f"Your REM sleep was low ({rem:.0f}%)",
            [f"At {rem:.0f}%, your brain had less time to process emotions and consolidate memories",
             "Low REM leaves you feeling emotionally drained and mentally foggy tomorrow",
             "Caffeine, stress, and interrupted sleep all reduce your REM time"],
            "Protect your last 2 hours of sleep — don't set your alarm earlier than needed. That's your peak REM window.")
    if f1=="deep_sleep_percentage":
        return (f"Your deep sleep was low ({deep:.0f}%)",
            [f"At {deep:.0f}%, your body had less time for physical repair — muscles, immune system, energy",
             "Without enough deep sleep, you wake up feeling tired even after enough hours in bed",
             "Most deep sleep happens in the first half of the night — your bedtime timing matters"],
            "Exercise earlier in the day — not within 3 hours of bed. It's the most reliable way to get more deep sleep.")
    if f1=="exercise_day":
        ex_val=u.get("exercise_day",0)
        if ex_val==1:
            return (f"Exercise today helped your sleep",
                [f"You exercised today — this increases deep sleep and reduces the time it takes to fall asleep",
                 f"With {steps:,} steps, your body built up natural physical tiredness",
                 "Physical activity is one of the most reliable ways to improve sleep quality"],
                "Keep this up — exercising consistently improves your sleep score every week.")
        else:
            return (f"No exercise today ({steps:,} steps)",
                [f"Without physical activity, your body isn't physically tired enough for deep sleep tonight",
                 f"You took {steps:,} steps — the target is 7,000+ to build enough natural tiredness for quality sleep",
                 "Low movement days consistently result in lighter, less satisfying sleep"],
                "Tomorrow: a 25-minute walk before 6 PM. This single change will show up in your sleep score.")
    if f1=="room_temperature_celsius":
        return (f"Your bedroom was {temp}°C — too warm",
            [f"At {temp}°C, your room is above the ideal 17–20°C range for deep sleep",
             "Your body needs to cool down slightly to enter deep sleep — a warm room actively prevents this",
             "This is one of the easiest and highest-impact sleep fixes available"],
            f"Open a window or use a fan to reach 18–20°C tonight. Keep feet outside the covers to cool faster.")
    if f1=="work_hours_that_day":
        return (f"You worked {work:.0f} hours today",
            [f"After {work:.0f} hours of work, your brain stays in 'problem-solving mode' for hours afterward",
             "This makes it harder to fall asleep and results in lighter early-night sleep",
             "Your brain needs a clear, intentional 'off switch' between work and sleep"],
            "Create a shutdown ritual: close everything, write tomorrow's 3 priorities, then stop completely.")
    if f1=="steps_that_day":
        return (f"Low movement today ({steps:,} steps)",
            [f"At {steps:,} steps, your body didn't accumulate enough physical tiredness for deep sleep",
             "Physical tiredness is one of the main signals your body uses to sleep deeply",
             "Desk-bound days almost always result in lighter, less satisfying sleep"],
            "Tomorrow: take stairs, walk during calls, park further. 7,000 steps makes a measurable difference.")
    if f1=="nap_duration_mins":
        if nap==0:
            return (f"No nap today — good for tonight's sleep",
                ["Skipping naps preserves your natural sleep pressure for bedtime",
                 "Your body will be more ready for deep sleep tonight",
                 "This is one of the healthy habits that supports good sleep quality"],
                "Keep this pattern — consistent no-nap days lead to deeper sleep at night.")
        elif nap<=30:
            return (f"Short nap today ({nap:.0f} min) — this is fine",
                [f"A {nap:.0f}-minute nap is within the healthy range and shouldn't affect tonight's sleep",
                 "Short naps (under 30 min) don't enter deep sleep, so your sleep pressure stays intact",
                 "You should still be ready to sleep well at your target bedtime"],
                "Keep naps short (under 30 min) and before 3 PM — you're doing it right.")
        else:
            return (f"Your {nap:.0f}-minute nap is working against tonight's sleep",
                [f"Naps over 30 minutes enter deep sleep — which reduces how tired you feel at bedtime",
                 f"Your {nap:.0f}-minute nap reduced your natural sleep pressure for tonight",
                 "You've essentially spent some of tonight's sleep budget during the day"],
                "Keep naps to 20 min max, before 3 PM. Set an alarm before lying down.")
    # fallback
    fname=f1.replace("_"," ").title()
    return (fname,
        [f"This factor had a significant impact on your sleep quality tonight",
         "Improving this habit consistently will raise your sleep score over time"],
        "Focus on this area this week and check your next sleep score.")

# Legacy dict for backward compat — now replaced by get_sab_info()
SAB_MAP={}
...
def get_sleep_health_notes(p, u):
    """Return a list of (icon, title, body) sleep health notes tailored to the result."""
    sq = p["sleep_quality"]; cog = p["cognitive"]; score = p["score"]
    hrs = u.get("sleep_duration_hrs", 7); stress = u.get("stress_score", 5)
    deep = u.get("deep_sleep_percentage", 18); rem = u.get("rem_percentage", 20)
    lat = u.get("sleep_latency_mins", 15); wk = u.get("wake_episodes_per_night", 1)
    caf = u.get("caffeine_mg_before_bed", 0); screen = u.get("screen_time_before_bed_mins", 0)
    steps = u.get("steps_that_day", 7000); temp = u.get("room_temperature_celsius", 20)
    mh = str(u.get("mental_health_condition", "none")).lower()
    dis = p.get("disorder", "none").lower()
    notes = []

    # ── Sleep quality tier notes ────────────────────────────────────────────
    if sq >= 8.5:
        notes.append(("✦", "Elite sleep — your body is fully restored",
            "A score above 8.5 means your brain completed all four sleep stages multiple times. You woke up with fully cleared adenosine, consolidated memories, and repaired muscle tissue. Nights like this are the foundation of peak physical and mental performance."))
    elif sq >= 7.5:
        notes.append(("✦", "Solid restorative sleep last night",
            "Above 7.5 means your body completed enough sleep cycles to reach meaningful deep and REM sleep. Your immune system, memory, and emotional regulation all benefited. Keep these habits — consistency amplifies the effect."))
    elif sq >= 6.5:
        notes.append(("◑", "Good sleep, with room to go deeper",
            "You got above-average sleep, but your deep and REM stages were likely shorter than ideal. The gap between 6.5 and 8+ quality isn't just numbers — it's how rested you feel by 2 PM. Small habit shifts can close that gap quickly."))
    elif sq >= 5.5:
        notes.append(("◑", "Moderate sleep — your body partially recovered",
            "At this quality, your brain and body got some restoration, but not complete. You likely had disrupted cycles or insufficient deep sleep. Over multiple nights like this, cognitive fatigue builds silently — you stop noticing how much sharper you could feel."))
    elif sq >= 4:
        notes.append(("◎", "Below-average sleep — recovery was incomplete",
            "Your body needed more than it got last night. Incomplete sleep cycles mean elevated stress hormones, slower reaction times, and emotional volatility today. Focus on the biggest fixable factor — usually stress, screens, or inconsistent timing."))
    else:
        notes.append(("◎", "Poor sleep — your body is running on deficit",
            "Very low sleep quality means multiple disruptions to your sleep architecture. Your adenosine clearance is incomplete, cortisol is elevated, and your prefrontal cortex is operating at reduced capacity. Prioritise tonight's sleep above everything else."))

    # ── Cognitive performance note ──────────────────────────────────────────
    if cog >= 80:
        notes.append(("✦", "Your brain is sharp today — protect that focus",
            f"At {cog:.0f}/100, your prefrontal cortex is well-rested. You're at your best for complex decisions, creative thinking, and sustained deep work. Schedule your hardest tasks in the first half of the day — this cognitive window is finite."))
    elif cog >= 65:
        notes.append(("◑", f"Decent focus today — watch for the afternoon dip",
            f"At {cog:.0f}/100, you're functional and productive. However, the post-lunch circadian dip (1–3 PM) will hit harder than usual. If you can, schedule demanding tasks before noon and lighter work after."))
    elif cog >= 50:
        notes.append(("◑", "Reduced focus — break your work into shorter blocks",
            f"At {cog:.0f}/100, your working memory is running below capacity. Sustained concentration will be harder. Use the Pomodoro method: 25 minutes of focused work, then a 5-minute break. Avoid major decisions until tomorrow if possible."))
    else:
        notes.append(("◎", "Low cognitive function today — compensate deliberately",
            f"At {cog:.0f}/100, you're operating with significant impairment. Studies show that below this threshold, reaction time is comparable to mild intoxication — yet we don't perceive it that way. A 20-minute nap between 1–3 PM can restore 30–40% of lost cognitive performance."))

    # ── Stress-specific note ────────────────────────────────────────────────
    if stress >= 8:
        notes.append(("✦", "High stress is the silent killer of deep sleep",
            "At 8+/10 stress, your body produces elevated cortisol throughout the night — directly suppressing melatonin and deep sleep production. The result is more time in light sleep and more awakenings, even if you don't remember them. A 10-minute writing session before bed (brain dump everything on your mind) reduces pre-sleep arousal and measurably improves sleep quality."))
    elif stress >= 6:
        notes.append(("◑", "Moderate stress reduces sleep depth more than you realise",
            f"Stress at {stress}/10 keeps your nervous system slightly activated at bedtime — pushing your brain toward light sleep rather than deep slow-wave sleep. Even a 5-minute slow breathing exercise (4 seconds in, hold 4, out 6) activates the parasympathetic nervous system and helps your body transition."))

    # ── Duration note ──────────────────────────────────────────────────────
    if hrs < 5.5:
        notes.append(("◎", "Severe sleep restriction — this has measurable consequences",
            f"At {hrs:.1f} hours, you've completed fewer than 4 sleep cycles. Research shows that sleeping under 6 hours for just 5 consecutive nights impairs performance equivalently to two full nights without sleep — but you stop feeling as impaired as you actually are. Tonight is critical."))
    elif hrs < 7:
        notes.append(("◑", f"Sleep debt is accumulating ({hrs:.1f} hrs last night)",
            "Each hour under 7 is one fewer recovery cycle. Over a week, the cognitive deficit adds up to losing an entire night's sleep. The good news: going to bed just 30 minutes earlier consistently is enough to reverse most short-term sleep debt."))
    elif hrs >= 9.5:
        notes.append(("◑", f"Long sleep ({hrs:.1f} hrs) — could signal accumulated debt or disrupted cycles",
            "Sleeping very long occasionally is normal. But consistently needing 9+ hours often signals poor sleep quality (your body compensating with time), accumulated sleep debt, or an underlying issue worth discussing with a doctor."))

    # ── Sleep architecture notes ─────────────────────────────────────────────
    if deep < 12:
        notes.append(("◑", "Low deep sleep — your body's physical repair was cut short",
            "Deep slow-wave sleep is when growth hormone is released, muscles repair, and your immune system strengthens. At less than 12%, your body didn't get enough of this. Alcohol, a warm room, and late exercise are the most common causes. Exercise earlier in the day is the single most reliable way to increase deep sleep percentage."))
    elif deep >= 25:
        notes.append(("✦", "Excellent deep sleep — your body repaired itself thoroughly",
            f"At {deep:.0f}% deep sleep, your growth hormone release, tissue repair, and immune function were at their peak. This is the physical foundation of feeling genuinely recovered rather than just 'not tired'."))

    if rem < 15:
        notes.append(("◑", "Low REM sleep — emotional and memory processing was reduced",
            "REM is when your brain processes emotional experiences, consolidates memories, and runs creative problem-solving. Under 15% means shorter or interrupted REM cycles. Caffeine, stress, and alarm interruptions are the main culprits. Protecting your last 90 minutes of sleep — by not setting your alarm earlier than necessary — preserves your REM window."))

    # ── Circadian and timing note ───────────────────────────────────────────
    if lat > 30:
        notes.append(("◑", "Long sleep onset — your brain wasn't ready for sleep",
            f"Taking {lat:.0f} minutes to fall asleep suggests your circadian phase wasn't aligned with your bedtime, or your nervous system was too activated. This is often caused by inconsistent sleep timing, light exposure, or late stimulants. The fix: same bedtime every night, and dim your lights for 60 minutes before bed."))

    # ── Disorder-specific notes ─────────────────────────────────────────────
    if "insomnia" in dis:
        notes.append(("◎", "Insomnia pattern detected — but it's highly treatable",
            "Insomnia is defined as difficulty initiating or maintaining sleep at least 3 nights/week for 3+ months, with daytime impairment. Cognitive Behavioral Therapy for Insomnia (CBT-I) has an 80%+ success rate — higher than sleep medications — with lasting results. Start with two CBT-I principles tonight: only get into bed when sleepy, and if awake for 20+ minutes, get up and do something calm until sleepy again."))
    elif "apnea" in dis:
        notes.append(("◎", "Sleep apnea markers in your profile — worth investigating",
            "Sleep apnea causes repeated micro-arousals (often unremembered) that prevent deep sleep, leaving you unrefreshed despite adequate hours. Classic signs include morning headaches, waking to urinate, and a partner reporting snoring or gasping. A sleep study can confirm it definitively. CPAP therapy, when used, dramatically improves sleep quality and cardiovascular health."))

    # ── Mental health note ─────────────────────────────────────────────────
    if mh == "anxiety" and sq < 7:
        notes.append(("✦", "Anxiety and sleep have a two-way relationship",
            "Anxiety disrupts sleep — and poor sleep worsens anxiety. Sleep deprivation amplifies the amygdala's threat response and weakens the prefrontal cortex's ability to regulate it. This is why improving your sleep quality is one of the most effective anxiety interventions. CBT-I (for insomnia) and CBT-A (for anxiety) share overlapping techniques that help both simultaneously."))
    elif mh == "depression" and sq < 7:
        notes.append(("✦", "Sleep and depression — treating one helps the other",
            "Depression disrupts sleep architecture — particularly reducing deep sleep and causing early morning awakening. But it also works the other way: poor sleep is a leading predictor of depressive episodes. Improving sleep consistency and quality can measurably reduce depressive symptoms. Fixed wake times (even on weekends) are the single most powerful circadian intervention for mood regulation."))

    # ── Positive reinforcement note ─────────────────────────────────────────
    if score >= 75 and stress <= 5 and caf <= 100:
        notes.append(("✦", "Your habits are working — this is what consistency looks like",
            "Low stress, moderate caffeine, and a good sleep score don't happen by accident. This is your sleep system working the way it's designed to. The most important thing now isn't to optimise further — it's to not break the pattern. Keep the same bedtime this week."))
    elif score >= 60:
        notes.append(("◑", "You're trending in the right direction",
            f"A score of {score} puts you in the 'Good' range. The difference between 60 and 80 is usually one or two consistent habit changes — typically bedtime consistency and either stress reduction or caffeine timing. Small, sustainable changes compound fast with sleep."))

    # ── Temperature note ─────────────────────────────────────────────────
    if temp > 23:
        notes.append(("◎", f"Room temperature ({temp}°C) is disrupting your sleep architecture",
            "Your core body temperature needs to drop by 1–2°C to initiate and maintain deep sleep. A warm room prevents this thermoregulatory drop, keeping you in lighter sleep stages all night. The optimal sleep temperature is 18–20°C. Open a window, use a fan, or set AC before bed — it's one of the fastest and most impactful sleep improvements you can make."))
    elif temp <= 19:
        notes.append(("✦", f"Your room temperature ({temp}°C) is in the ideal sleep zone",
            "Cool sleeping environments directly facilitate the core body temperature drop that triggers deep sleep onset. You're doing this right — it's one of the most overlooked and most effective sleep factors."))

    # ── Movement note ────────────────────────────────────────────────────
    if steps >= 9000:
        notes.append(("✦", f"High activity today ({steps:,} steps) — expect deeper sleep tonight",
            "Physical activity builds adenosine — the chemical that creates sleep pressure. High step counts are consistently associated with faster sleep onset, more deep sleep, and higher overall sleep quality. Your body has a reason to sleep deeply tonight."))
    elif steps < 3000:
        notes.append(("◎", f"Very low movement today ({steps:,} steps)",
            "Sedentary days reduce adenosine buildup, meaning your body has less biological drive to sleep deeply. This isn't about calories — it's about the sleep-promoting effect of physical tiredness. Even a 20-minute walk tomorrow morning will help."))

    # Return a curated selection — variety ensures notes feel fresh
    import random
    random.seed(int(sq * 10 + score))  # deterministic per result, not fully random
    if len(notes) > 5:
        # Always keep the first (quality tier) and second (cognitive) notes, sample the rest
        core = notes[:2]
        extras = notes[2:]
...
# ═══════════════════════════════════════════════════════════
with st.spinner(""):
    df_raw=load_data(); M=train_all(df_raw)
...
# ═══════════════════════════════════════════════════════════ HOME
    ...
    st.markdown(f"""<div style='padding:.5rem 0 1.5rem'>
      ...
      <h1 style='font-family:Cormorant Garamond,serif;font-size:3rem;font-weight:300;color:#e8e0ff;margin-top:.3rem'>
        {_greet}, <em style='color:#c084fc'>{name}</em> 🌙</h1>
...
# ═══════════════════════════════════════════════════════════ CHECK-IN
    ...
    _date_str=_today.strftime("%A, %B %-d, %Y")
    ...
    if submitted:
        ...
        entry={"date":_today.strftime("%b %-d"),"score":result["score"],"sq":result["sleep_quality"],"bedtime":result["bedtime"],"cog":result["cognitive"]}
...
# ═══════════════════════════════════════════════════════════ RESULTS
    ...
    if "result" not in st.session_state or not st.session_state.get("result"):
        ...
        p=st.session_state["result"]; u=st.session_state["user"]
        sq=p["sleep_quality"]; cog=p["cognitive"]
        sq_lbl,sq_col=sq_label(sq); cog_lbl,cog_col=cog_label(cog)
        score_lbl,score_col=score_label(p["score"])
        dis=p["disorder"]; is_ok=dis.lower() in ["none","no disorder"]
        dis_col="#a78bfa" if is_ok else "#f87171"
        _today_str=datetime.date.today().strftime("%A, %B %-d")

        st.markdown("""<script>window.scrollTo(0,0);</script>""",unsafe_allow_html=True)
        st.markdown(f"""<p style='font-size:.72rem;letter-spacing:3px;color:#7c6fa0;text-transform:uppercase'>{_today_str} · Sleep Report</p>
        <h2 style='font-family:Cormorant Garamond,serif;font-size:2.4rem;font-weight:300;color:#e8e0ff;margin-top:.3rem;margin-bottom:1.5rem'>Your Results</h2>""",unsafe_allow_html=True)

        # ── 4 metric cards — full width row ──────────────────────────────
        def mk(label,num_html,word,word_col,bar_pct,pts):
            bar_col=word_col
            bullets="".join(f"<li style='font-size:.79rem;color:#9d8ec7;line-height:1.5;padding:.22rem 0;display:flex;gap:.45rem;align-items:flex-start'><span style='color:#c084fc;font-size:.6rem;margin-top:.25rem;flex-shrink:0'>◑</span>{pt}</li>" for pt in pts)
        ...
        def bullet_li(txt): return f"<li style='font-size:.79rem;color:#9d8ec7;line-height:1.5;padding:.22rem 0;display:flex;gap:.45rem;align-items:flex-start'><span style='color:#c084fc;font-size:.6rem;margin-top:.2rem;flex-shrink:0'>◑</span>{txt}</li>"

        r_col2="#7dd3a8" if p["felt_rested"] else "#f87171"
        dis_lower2=dis.lower().strip()
        is_ok_robust=(dis_lower2 in ["none","no disorder",""] or dis_lower2.startswith("none"))
        dis_show2="Healthy" if is_ok_robust else dis.replace("_"," ").title()
        dis_col2="#a78bfa" if is_ok_robust else "#f87171"
        hrs=u.get("sleep_duration_hrs",7); lat=u.get("sleep_latency_mins",15)
        wk=u.get("wake_episodes_per_night",1); deep=u.get("deep_sleep_percentage",18)
        stress=u.get("stress_score",5); caf=u.get("caffeine_mg_before_bed",0)
        screen=u.get("screen_time_before_bed_mins",0); temp=u.get("room_temperature_celsius",20)

        # Build bullet points — short, plain English
        sq_pts=[]
        if sq>=7: sq_pts.append("You slept well — your body got the recovery it needed")
        elif sq>=5: sq_pts.append("Okay sleep, but not fully restorative")
        else: sq_pts.append("Poor sleep — your body didn't fully recover last night")
        if hrs<6: sq_pts.append(f"Only {hrs:.1f} hrs slept — you need at least 7 for full recovery")
        if lat>25: sq_pts.append(f"Took {lat:.0f} min to fall asleep — mind was still active at bedtime")
        ...
        if not sq_pts[1:]: sq_pts.append(f"Slept {hrs:.1f} hrs · woke {wk} time(s) · fell asleep in {lat:.0f} min")

        cog_pts=[]
        if cog>=75: cog_pts.append(f"Brain is well-rested — expect sharp focus today ({cog:.0f}/100)")
        elif cog>=60: cog_pts.append(f"Decent focus today ({cog:.0f}/100) — may feel slower after 3 PM")
        else: cog_pts.append(f"Focus will be harder than usual today ({cog:.0f}/100) — take more breaks")
        if hrs<6: cog_pts.append(f"Only {hrs:.1f} hrs of sleep significantly reduces memory and reaction speed")
        elif hrs<7: cog_pts.append(f"At {hrs:.1f} hrs, you're slightly under the 7-hour optimal for full cognitive performance")
        if stress>=7: cog_pts.append(f"Stress at {stress}/10 also reduces cognitive performance independently of sleep")
        if cog<60: cog_pts.append("A 20-min nap between 1–3 PM can partially restore focus today")

        rest_pts=[]
        if p["felt_rested"]:
            rest_pts=[f"Your sleep quality ({sq:.1f}/10) and duration ({hrs:.1f} hrs) are sufficient for morning alertness","You should feel refreshed when you wake up"]
            if hrs>=7 and sq>=7: rest_pts.append("This is a good night — keep these habits consistent")
        else:
            if hrs<5.5: rest_pts.append(f"Only {hrs:.1f} hrs — your body simply didn't have enough time to rest")
            elif hrs<7: rest_pts.append(f"At {hrs:.1f} hrs, you're just under the threshold for feeling fully rested")
            if wk>=3: rest_pts.append(f"Waking {wk} times prevented your body from reaching the deep rest it needs")
            if sq<5: rest_pts.append(f"Sleep quality of {sq:.1f}/10 means your sleep stages were disrupted")
            if not rest_pts: rest_pts.append(f"Sleep quality ({sq:.1f}/10) wasn't enough for full morning restoration")
            rest_pts.append("Expect some grogginess for 20–30 min after waking")

        dis_pts=[]
        np2=p["none_prob"]; conf=int(np2*100)
        dis_lower_check=dis.lower().strip()
        is_healthy=(dis_lower_check in ["none","no disorder",""] or dis_lower_check.startswith("none"))
        if is_healthy:
            dis_pts.append(f"No sleep disorder detected — your pattern looks healthy ({conf}% confidence)")
            if conf>=80: dis_pts.append("Your sleep patterns are regular and consistent")
            else: dis_pts.append("Continue tracking nightly to build a fuller picture")
        elif "insomnia" in dis_lower_check:
            dis_pts.append("Your pattern shows signs consistent with insomnia")
            if lat>25: dis_pts.append(f"Taking {lat:.0f} min to fall asleep is above the healthy 10–20 min range")
            if wk>=3: dis_pts.append(f"Waking {wk} times per night is a key marker")
            dis_pts.append("A consistent pre-bed routine and fixed wake time helps most")
        elif "apnea" in dis_lower_check:
            dis_pts.append("Your profile shows markers associated with sleep apnea")
            dis_pts.append(f"Waking {wk} times and not feeling rested are the main signs")
            dis_pts.append("Very treatable — a doctor visit is worth it")
        else:
            dis_pts.append("Your sleep pattern shows some mild markers worth monitoring")
            dis_pts.append("Consistent sleep and wake times for 2–3 weeks usually helps")

        c1,c2,c3,c4=st.columns(4,gap="small")
        with c1: st.markdown(mk("Sleep Quality",f"{sq:.1f}<span style='font-size:1rem;color:#6d5fa0'>/10</span>",sq_lbl,sq_col,int(sq/10*100),sq_pts),unsafe_allow_html=True)
        with c2: st.markdown(mk("Focus Tomorrow",f"{cog:.0f}<span style='font-size:1rem;color:#6d5fa0'>/100</span>",cog_lbl,cog_col,int(cog),cog_pts),unsafe_allow_html=True)
        with c3: st.markdown(mk("Morning Feeling","Rested" if p["felt_rested"] else "Tired","",r_col2,100 if p["felt_rested"] else 30,rest_pts),unsafe_allow_html=True)
        with c4: st.markdown(mk("Sleep Health",dis_show2,"",dis_col2,int(np2*100),dis_pts),unsafe_allow_html=True)
        ...
        with sc:
            st.markdown("<p style='font-family:Cormorant Garamond,serif;font-size:1.3rem;color:#e8e0ff;margin-bottom:.3rem'>What hurt your sleep tonight?</p><p style='font-size:.8rem;color:#7c6fa0;margin-bottom:.9rem'>Based on what you entered today — these are the biggest reasons your sleep was affected.</p>",unsafe_allow_html=True)
            if p.get("saboteurs"):
                for sab in p["saboteurs"]:
                    f1=sab["features"][0]
                    sab_name,sab_pts,sab_act=get_sab_info(f1,u)
                    pts_li="".join(f"<li style='font-size:.8rem;color:#9d8ec7;line-height:1.5;padding:.22rem 0;display:flex;gap:.5rem;align-items:flex-start'><span style='color:#f87171;font-size:.6rem;margin-top:.2rem;flex-shrink:0'>◑</span>{pt}</li>" for pt in sab_pts)
                    st.markdown(f"""<div style='padding:1.3rem 1.5rem;border-radius:18px;background:rgba(248,113,113,.05);
                      ...
                      <div style='font-size:.6rem;letter-spacing:2px;color:#f87171;text-transform:uppercase;margin-bottom:.35rem'>Biggest factor tonight · {sab["impact"]:.0f}% impact</div>
                ...
                st.markdown("""<div style='padding:1.3rem 1.5rem;border-radius:18px;background:rgba(125,211,168,.05);border:1px solid rgba(125,211,168,.2)'>
                  <div style='font-size:.6rem;letter-spacing:2px;color:#7dd3a8;text-transform:uppercase;margin-bottom:.35rem'>Great news</div>
                  <div style='font-size:.9rem;color:#a7f3d0;font-weight:600;margin-bottom:.4rem'>No major sleep disruptors tonight</div>
                  <div style='font-size:.8rem;color:#9d8ec7'>Your habits today didn't show strong negative factors. Keep this up — consistency is the key to improving your score over time.</div>
                </div>""",unsafe_allow_html=True)

        st.markdown("<div class='divider'></div>",unsafe_allow_html=True)

        # ── Tips 2-column ─────────────────────────────────────────────────
        st.markdown("<p style='font-family:Cormorant Garamond,serif;font-size:1.6rem;color:#e8e0ff;margin-bottom:.3rem'>Your Sleep Tips Tonight</p><p style='font-size:.82rem;color:#7c6fa0;margin-bottom:1.2rem'>Based on your data today — written specifically for you.</p>",unsafe_allow_html=True)
        ...
        for col,val,lbl,clr in [(s1,n_nights,"Nights tracked","#c084fc"),(s2,n_good,"Good nights (60+)","#7dd3a8"),(s3,avg_sc,"Your avg score","#f4c97a"),(s4,best_sc,"Your best score","#a78bfa")]:
            with col:
        ...
        # ── Sleep Health Notes ─────────────────────────────────────────────
        st.markdown("<div class='divider'></div>",unsafe_allow_html=True)
        st.markdown("<p style='font-family:Cormorant Garamond,serif;font-size:1.6rem;color:#e8e0ff;margin-bottom:.3rem'>Sleep Health Notes</p><p style='font-size:.82rem;color:#7c6fa0;margin-bottom:1.2rem'>Personalised insights based on your results — the science behind what your body experienced last night.</p>",unsafe_allow_html=True)
        health_notes=get_sleep_health_notes(p,u)
        hn1,hn2=st.columns(2,gap="medium")
        for i,(icon,title,body) in enumerate(health_notes):
            col=hn1 if i%2==0 else hn2
            with col:
                border_clr="rgba(167,139,250,.25)" if icon=="✦" else "rgba(192,132,252,.13)"
                bg_clr="rgba(124,58,237,.06)" if icon=="✦" else "rgba(192,132,252,.04)"
                icon_clr="#a78bfa" if icon=="✦" else "#c084fc"
                st.markdown(f"""<div style='display:flex;gap:1rem;align-items:flex-start;padding:1.3rem 1.4rem;
                    border-radius:18px;background:{bg_clr};border:1px solid {border_clr};margin-bottom:.8rem'>
                  <div style='font-size:1.2rem;flex-shrink:0;padding-top:2px;color:{icon_clr}'>{icon}</div>
                  <div>
                    <div style='font-size:.91rem;color:#e8e0ff;font-weight:600;margin-bottom:.3rem'>{title}</div>
                    <div style='font-size:.81rem;color:#9d8ec7;line-height:1.7'>{body}</div>
                  </div>
                </div>""",unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════ TIPS (standalone)
    ...
    if "result" not in st.session_state or not st.session_state.get("result"):
        ...
        with t1:
            for sym,title,body,action in tips:
                st.markdown(f"""<div class='tip-row'><div class='tip-icon'>{sym}</div>
                  ...
                  <div class='tip-action'>{action}</div></div></div>""",unsafe_allow_html=True)
        with t2:
        ...
        # ── Sleep Health Notes on Tips Page ─────────────────────────────────
        st.markdown("<div class='divider'></div>",unsafe_allow_html=True)
        st.markdown("<p style='font-family:Cormorant Garamond,serif;font-size:1.6rem;color:#e8e0ff;margin-bottom:.3rem'>Sleep Health Notes</p><p style='font-size:.82rem;color:#7c6fa0;margin-bottom:1.2rem'>The science behind what your body experienced last night — personalised to your data.</p>",unsafe_allow_html=True)
        health_notes_t=get_sleep_health_notes(p,u)
        for icon,title,body in health_notes_t:
            border_clr="rgba(167,139,250,.25)" if icon=="✦" else "rgba(192,132,252,.13)"
            bg_clr="rgba(124,58,237,.06)" if icon=="✦" else "rgba(192,132,252,.04)"
            icon_clr="#a78bfa" if icon=="✦" else "#c084fc"
            st.markdown(f"""<div style='display:flex;gap:1rem;align-items:flex-start;padding:1.3rem 1.5rem;
                border-radius:18px;background:{bg_clr};border:1px solid {border_clr};margin-bottom:.8rem'>
              <div style='font-size:1.2rem;flex-shrink:0;padding-top:2px;color:{icon_clr}'>{icon}</div>
              <div>
                <div style='font-size:.91rem;color:#e8e0ff;font-weight:600;margin-bottom:.3rem'>{title}</div>
                <div style='font-size:.82rem;color:#9d8ec7;line-height:1.7'>{body}</div>
              </div>
            </div>""",unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════ EXPLORE
    ...
    with t1:
        ...
        st.markdown("""<div class='article-section' style='margin-top:1.5rem'>
          ...
          <div class='article-body'>Sleep deprivation is deceptive — you stop noticing how impaired you actually are. Your self-assessment of performance stays higher than your real performance as you get more sleep-deprived. Chronic sleep restriction of 6 hours per night for two weeks produces the same cognitive deficit as 48 hours of total sleep deprivation. And you don't fully recover in one night — it takes 4–5 nights of full sleep to restore cognitive function completely.</div>
        </div>""",unsafe_allow_html=True)
    ...
    with t2:
        ...
        st.markdown(f"""<div class='card-purple'>
          <div style='font-size:.65rem;letter-spacing:2px;color:#c084fc;text-transform:uppercase;margin-bottom:.6rem'>Key Finding</div>
          <div style='font-size:.9rem;color:#d4c8f5;line-height:1.9'>People with stress above 7/10 average <strong style='color:#e8e0ff'>{df[df["stress_score"]>=7]["sleep_quality_score"].mean():.1f}/10</strong> sleep quality, compared to <strong style='color:#e8e0ff'>{df[df["stress_score"]<=3]["sleep_quality_score"].mean():.1f}/10</strong> for those with stress at 3 or below. That's a {abs(df[df["stress_score"]>=7]["sleep_quality_score"].mean()-df[df["stress_score"]<=3]["sleep_quality_score"].mean()):.1f}-point difference from stress alone.</div>
        </div>""",unsafe_allow_html=True)
